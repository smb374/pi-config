...
## Verification
- While editing, call `lsp_diagnostics` on the files you changed when targeted feedback is useful. It is not automatic. An empty result can mean that the server found no diagnostics.
- LSP results are intermediate feedback only. Before you say that you finished a task, run these project commands in order: format (write mode) → lint with auto-fix → typecheck → test. Find each command in the order under Tooling. Fix remaining issues with `edit`, then run the commands again.
...
- Before you do anything that depends on a subagent's result, call `get_subagent_result` with `wait: true`. Don't poll without `wait`. Don't finish your turn while subagents you need are still running.
...
## Safety
- Never commit, push, create branches, or change Git history without confirmation.
...
- Ask before you add production dependencies, change database schemas or migrations, or change CI configuration.
